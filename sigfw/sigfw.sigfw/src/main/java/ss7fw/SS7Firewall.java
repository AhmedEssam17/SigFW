...
/**
 * Main SS7Firewall class. Class contains mainly static variables used to build
 * the SS7 stacks.
 * 
 * @author Martin Kacer
 */
public class SS7Firewall
        implements ManagementEventListener, Mtp3UserPartListener, SccpListener, TCListener, MAPServiceListener,
        MAPDialogListener, MAPServiceSupplementaryListener, MAPServiceMobilityListener, MAPServiceCallHandlingListener,
        MAPServiceLsmListener, MAPServiceOamListener, MAPServicePdpContextActivationListener, MAPServiceSmsListener {
    ...
    // Just store first N (encryption_autodiscovery_digits) digits of GT to do not
    // spam the foreign PLMN
    // Key: Called_GT
    // Value: TID
    ...
    private static int DTLS_MAX_HANDSHAKE_DURATION = 10; // in seconds, after the handshake SSL engine is dropped. Has
                                                         // to be shorter than half of DTLS_MAX_SESSION_DURATION
    ...
    private static String dtls_keyFilename = System.getProperty("test.src", ".") + "/" + dtls_pathToStores +
            "/" + dtls_keyStoreFile;
    private static String dtls_trustFilename = System.getProperty("test.src", ".") + "/" + dtls_pathToStores +
            "/" + dtls_trustStoreFile;
    ...
    // 2 DTLS sessions, in and out. Server side is used for decrypt, client side for
    // encrypt.
    private static Map<String, SSLEngine> dtls_engine_permanent_server = new ConcurrentHashMap<>(); // <peer_realm,
                                                                                                    // SSLEngine>
    private static Map<String, SSLEngine> dtls_engine_permanent_client = new ConcurrentHashMap<>(); // <peer_realm,
                                                                                                    // SSLEngine>
    ...
    // private static Map<String, Thread> dtls_handshake_treads =
    // ExpiringMap.builder()
    // .expiration(DTLS_MAX_HANDSHAKE_DURATION, TimeUnit.SECONDS)
    // .build(); // <peer_realm, Thread>

    // DTLS client initialization timer, do not initiate new DTLS handshake till
    // this timer
    // Value: Dest_Realm
    // Key: E2E ID
    ...
    private static Map<String, ConcurrentLinkedQueue<DatagramOverSS7Packet>> datagramOverSS7Socket_inbound_server = ExpiringMap
            .builder()
            .expiration(DTLS_MAX_HANDSHAKE_DURATION, TimeUnit.SECONDS)
            .build(); // <peer_realm, SSLEngine> //new ConcurrentHashMap<>(); //new
                      // ConcurrentLinkedQueue<>();
    private static Map<String, ConcurrentLinkedQueue<DatagramOverSS7Packet>> datagramOverSS7Socket_inbound_client = ExpiringMap
            .builder()
            .expiration(DTLS_MAX_HANDSHAKE_DURATION, TimeUnit.SECONDS)
            .build(); // <peer_realm, SSLEngine> //new ConcurrentHashMap<>(); //new
                      // ConcurrentLinkedQueue<>();
    // private static ConcurrentLinkedQueue<DatagramOverSS7Packet>
    // datagramOverSS7Socket_outbound = new ConcurrentLinkedQueue<>();
    ...
    private void initSCTP(IpChannelType ipChannelType) throws Exception {
        ...
        this.sctpManagement.setOptionSctpInitMaxstreams_MaxInStreams(
                Integer.parseInt((String) SS7FirewallConfig.get("$.sigfw_configuration.sctp.sctp_max_in_streams")));
        this.sctpManagement.setOptionSctpInitMaxstreams_MaxOutStreams(
                Integer.parseInt((String) SS7FirewallConfig.get("$.sigfw_configuration.sctp.sctp_max_out_streams")));
        ...
        List<Map<String, Object>> sctp_server_association = SS7FirewallConfig
                .get("$.sigfw_configuration.sctp.sctp_server_association");
        ...
        List<Map<String, Object>> sctp_association = SS7FirewallConfig
                .get("$.sigfw_configuration.sctp.sctp_association");
    ...
    private void initM3UA() throws Exception {
        ...
        for (int i = 0; i < SS7FirewallConfig.m3ua_server_remote_pc.size(); i++) {
            this.serverM3UAMgmt.addRoute(Integer.parseInt(SS7FirewallConfig.m3ua_server_remote_pc.get(i)), -1, -1,
                    (String) SS7FirewallConfig.get("$.sigfw_configuration.m3ua.m3ua_server.as_name"));
        }
        ...
        for (int i = 0; i < SS7FirewallConfig.m3ua_client_remote_pc.size(); i++) {
            this.clientM3UAMgmt.addRoute(Integer.parseInt(SS7FirewallConfig.m3ua_client_remote_pc.get(i)), -1, -1,
                    (String) SS7FirewallConfig.get("$.sigfw_configuration.m3ua.m3ua_client.as_name"));
        }
    ...
    private void initSCCP() throws Exception {
        ...
        for (int i = 0; i < SS7FirewallConfig.m3ua_server_remote_pc.size(); i++) {
            this.sccpStack.getSccpResource().addRemoteSpc(j,
                    Integer.parseInt(SS7FirewallConfig.m3ua_server_remote_pc.get(i)), 0, 0);
            this.sccpStack.getRouter().addMtp3ServiceAccessPoint(j, 2,
                    Integer.parseInt(SS7FirewallConfig.m3ua_server_remote_pc.get(i)), NETWORK_INDICATOR, 0);
            for (int k = 0; k < SS7FirewallConfig.m3ua_client_remote_pc.size(); k++) {
                // System.out.println("Server i " + i + " j " + j + " k " + k + " " +
                // m3ua_client_remote_pc.get(k));
                this.sccpStack.getRouter().addMtp3Destination(j, 1,
                        Integer.parseInt(SS7FirewallConfig.m3ua_client_remote_pc.get(k)),
                        Integer.parseInt(SS7FirewallConfig.m3ua_client_remote_pc.get(k)), 0, 255, 255);
            }
            j++;
        }
        ...
        for (int i = 0; i < SS7FirewallConfig.m3ua_client_remote_pc.size(); i++) {
            this.sccpStack.getSccpResource().addRemoteSpc(j,
                    Integer.parseInt(SS7FirewallConfig.m3ua_client_remote_pc.get(i)), 0, 0);
            this.sccpStack.getRouter().addMtp3ServiceAccessPoint(j, 1,
                    Integer.parseInt(SS7FirewallConfig.m3ua_client_remote_pc.get(i)), NETWORK_INDICATOR, 0);
            for (int k = 0; k < SS7FirewallConfig.m3ua_server_remote_pc.size(); k++) {
                // System.out.println("Client i " + i + " j " + j + " k " + k + " " +
                // m3ua_server_remote_pc.get(k));
                this.sccpStack.getRouter().addMtp3Destination(j, 2,
                        Integer.parseInt(SS7FirewallConfig.m3ua_server_remote_pc.get(k)),
                        Integer.parseInt(SS7FirewallConfig.m3ua_server_remote_pc.get(k)), 0, 255, 255);
            }
            j++;
        }
         ...
         * String mask;
         * 
         * // gt = this.sccpProvider.getParameterFactory().createGlobalTitle("", 0,
         * org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_MOBILE, null,
         * NatureOfAddress.INTERNATIONAL);
         * gt = this.sccpProvider.getParameterFactory().createGlobalTitle("11111111111",
         * 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
         * NatureOfAddress.INTERNATIONAL);
         * 
         * 
         * this.sccpStack.getRouter().addRoutingAddress(1,
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, SERVER_SPC, 0));
         * this.sccpStack.getRouter().addRoutingAddress(2,
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_DPC_AND_SSN, gt, CLIENT_SPC, 6));
         * this.sccpStack.getRouter().addRoutingAddress(3,
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_DPC_AND_SSN, gt, CLIENT_SPC, 7));
         * this.sccpStack.getRouter().addRoutingAddress(4,
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_DPC_AND_SSN, gt, CLIENT_SPC, 8));
         * 
         * SccpAddress pattern;
         * gt = this.sccpProvider.getParameterFactory().createGlobalTitle("11111111111",
         * 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
         ...
         * // pattern =
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 8);
         * // ((RouterImpl) this.sccpStack.getRouter()).addRule(1, RuleType.SOLITARY,
         * null, OriginationType.ALL, pattern, mask, 1, -1, null, 0);
         * //
         * 
         * mask = "*";
         * pattern =
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 8);
         * ((RouterImpl) this.sccpStack.getRouter()).addRule(1, RuleType.SOLITARY, null,
         * OriginationType.LOCAL, pattern, mask, 1, -1, null, 0);
         * pattern =
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 6);
         * ((RouterImpl) this.sccpStack.getRouter()).addRule(2, RuleType.SOLITARY, null,
         * OriginationType.LOCAL, pattern, mask, 1, -1, null, 0);
         * pattern =
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 7);
         * ((RouterImpl) this.sccpStack.getRouter()).addRule(3, RuleType.SOLITARY, null,
         * OriginationType.LOCAL, pattern, mask, 1, -1, null, 0);
         * 
         * 
         * mask = "*";
         ...
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 6);
         * ((RouterImpl) this.sccpStack.getRouter()).addRule(4, RuleType.SOLITARY, null,
         * OriginationType.REMOTE, pattern, mask, 1, -1, null, 0);
         * pattern =
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 7);
         * ((RouterImpl) this.sccpStack.getRouter()).addRule(5, RuleType.SOLITARY, null,
         * OriginationType.REMOTE, pattern, mask, 1, -1, null, 0);
         * pattern =
         * this.sccpProvider.getParameterFactory().createSccpAddress(RoutingIndicator.
         * ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, 8);
         * ((RouterImpl) this.sccpStack.getRouter()).addRule(6, RuleType.SOLITARY, null,
         * OriginationType.REMOTE, pattern, mask, 1, -1, null, 0);
         */
    ...
    private void initMAP() throws Exception {
         ...
         * 
         * this.mapProvider.getMAPServicePdpContextActivation().addMAPServiceListener(
         * this);
    ...
    public void initializeStack(IpChannelType ipChannelType) throws Exception {
        ...
        try {
            logger.info("Trying to load SigFW extensions from: " + "file://" + System.getProperty("user.dir")
                    + "/src/main/resources/SigFW_extension-1.0.jar");
            ...
            // Creating an instance of URLClassloader using the above URL and parent
            // classloader
            ClassLoader loader = URLClassLoader.newInstance(new URL[] { u },
                    ExternalFirewallRules.class.getClassLoader());

            // Returns the class object
            Class<?> mainClass = Class.forName("com.p1sec.sigfw.SigFW_extension.rules.ExtendedFirewallRules", true,
                    loader);
            externalFirewallRules = (FirewallRulesInterface) mainClass.getDeclaredConstructor().newInstance();

            // Returns the class object
            Class<?> mainClassCrypto = Class.forName("com.p1sec.sigfw.SigFW_extension.crypto.ExtendedCrypto", true,
                    loader);
     ...
     */
    private void sendSccpErrorMessage(Mtp3UserPart mup, int opc, int dpc, int sls, int ni, LongMessageRuleType lmrt,
            SccpDataMessage message, ReturnCauseValue returnCauseInt) {
        ...
        ReturnCause returnCause = ((ParameterFactory) this.sccpProvider.getParameterFactory())
        ...
        ans = this.sccpMessageFactory.createNoticeMessage(message.getType(), returnCause,
                message.getCallingPartyAddress(), message.getCalledPartyAddress(), message.getData(),
                message.getHopCounter(),
                message.getImportance());

        EncodingResultData erd;
        try {
            erd = ((SccpMessageImpl) ans).encode(this.sccpStack, lmrt, mup.getMaxUserDataLength(dpc), logger,
                    this.sccpStack.isRemoveSpc(),
                ...
                case Success:
                    ...
                    if (erd.getSolidData() != null) {
                        ...
                        for (byte[] bf : erd.getSegementedData()) {
                            Mtp3TransferPrimitive msg = factory.createMtp3TransferPrimitive(Mtp3._SI_SERVICE_SCCP, ni,
                                    0,
                                    opc, dpc, sls, bf);
                            mup.sendMessage(msg);
                        }
     ...
     * @param message Original SCCP message
     */
    private void sendSccpMessage(Mtp3UserPart mup, int opc, int dpc, int sls, int ni, LongMessageRuleType lmrt,
        ...
        try {
            if (SS7FirewallConfig.firewallPolicy == SS7FirewallConfig.FirewallPolicy.DNAT_TO_HONEYPOT
                ...
                if (message.getCallingPartyAddress() != null
                        ...
                        && message.getCallingPartyAddress().getGlobalTitle().getDigits()
                                .equals(SS7FirewallConfig.honeypot_sccp_gt)
                        && dnat_sessions.containsKey(message.getCalledPartyAddress().getGlobalTitle().getDigits())) {
                    String original_gt_ssn = dnat_sessions
                            .get(message.getCalledPartyAddress().getGlobalTitle().getDigits());
                    String[] gt_ssn = original_gt_ssn.split(":");
                    String original_gt = gt_ssn[0];
                    String original_ssn = gt_ssn[1];
                    GlobalTitle gt = this.sccpProvider.getParameterFactory().createGlobalTitle(original_gt, 0,
                            org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
                            NatureOfAddress.INTERNATIONAL);
                    SccpAddress sa = this.sccpProvider.getParameterFactory().createSccpAddress(
                            RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, Integer.decode(original_ssn));
                    message.setCallingPartyAddress(sa);
                }
                // Forward NAT towards Honeypot (for latter forward messages not detected as
                // alerts)
                    ...
                    dnat_sessions.put(message.getCallingPartyAddress().getGlobalTitle().getDigits(),
                            message.getCalledPartyAddress().getGlobalTitle().getDigits() + ":"
                                    + message.getCalledPartyAddress().getSubsystemNumber());
                    GlobalTitle gt = this.sccpProvider.getParameterFactory().createGlobalTitle(
                            SS7FirewallConfig.honeypot_sccp_gt, 0,
                            org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
                            NatureOfAddress.INTERNATIONAL);
                    SccpAddress sa_dnat = this.sccpProvider.getParameterFactory().createSccpAddress(
                            RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0,
                            message.getCalledPartyAddress().getSubsystemNumber());
                    message.setCalledPartyAddress(sa_dnat);
                }
            }

            erd = ((SccpMessageImpl) message).encode(this.sccpStack, lmrt, mup.getMaxUserDataLength(dpc), logger,
                    this.sccpStack.isRemoveSpc(),
                    this.sccpStack.getSccpProtocolVersion());
     ...
     */
    private void firewallMessage(Mtp3UserPart mup, Mtp3UserPart mupReturn, int opc, int dpc, int sls, int ni,
            LongMessageRuleType lmrt, SccpDataMessage message, String reason, HashMap<String, String> lua_hm) {
        ...
        if (SS7FirewallConfig.firewallPolicy == SS7FirewallConfig.FirewallPolicy.DROP_SILENTLY) {
            ...
            sendSccpErrorMessage(mupReturn, dpc, opc, sls, ni, lmrt, message,
                    ReturnCauseValue.NO_TRANSLATION_FOR_ADDRESS);
        } else if (SS7FirewallConfig.firewallPolicy == SS7FirewallConfig.FirewallPolicy.DNAT_TO_HONEYPOT
                && dnat_sessions != null
            ...
            dnat_sessions.put(session_key, message.getCalledPartyAddress().getGlobalTitle().getDigits() + ":"
                    + message.getCalledPartyAddress().getSubsystemNumber());
            message.setCalledPartyAddress(sa_dnat);
    ...
    public void onMessage(final SccpDataMessage msg) {
        ...
        threadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                for (Map.Entry<Integer, LongMessageRule> e : SS7Firewall.sccpStack.getRouter().getLongMessageRules()
                        .entrySet()) {
                ...
                if (message.getCallingPartyAddress() != null
                    ...
                    if (SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.sccp_calling_gt_whitelist,
                            message.getCallingPartyAddress().getGlobalTitle().getDigits())) {
                        logger.info("============ SCCP Whitelisted Calling GT = "
                                + message.getCallingPartyAddress().getGlobalTitle().getDigits() + " ============");
                        sendSccpMessage(mup, opc, dpc, sls, ni, lmrt, message);
                        return;
                    }

                    // logger.debug("SCCP Calling GT = " +
                    // message.getCallingPartyAddress().getGlobalTitle().getDigits());
                    if (SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.sccp_calling_gt_blacklist,
                            message.getCallingPartyAddress().getGlobalTitle().getDigits())) {

                        // logger.info("============ SCCP Blocked Calling GT = " +
                        // message.getCallingPartyAddress().getGlobalTitle().getDigits() + "
                        // ============");
                        firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message, "SCCP FW: Calling GT = "
                                + message.getCallingPartyAddress().getGlobalTitle().getDigits(), lua_hmap);
                        return;
                ...
                try {
                    ...
                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message, "TCAP FW: Missing TC tag",
                            lua_hmap);
                    return;
                }
                ...
                if (ais.getTagClass() != Tag.CLASS_APPLICATION) {
                    ...
                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message, "TCAP FW: Unrecognized TC tag",
                            lua_hmap);
                    return;
                }
                ...
                switch (tag) {
                    ...
                    case TCContinueMessage._TAG:
                        try {
                            ...
                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                    "TCAP FW: ParseException when parsing TCContinueMessage", lua_hmap);
                            return;
                        }
                    ...
                    case TCBeginMessage._TAG:
                        try {
                            ...
                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                    "TCAP FW: ParseException when parsing TCBeginMessage", lua_hmap);
                            return;
                        }
                        if (tcb.getDialogPortion() != null && tcb.getDialogPortion().getDialogAPDU() != null
                            ...
                            if (!dlg.getProtocolVersion().isSupportedVersion()) {
                                ...
                                firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                        "TCAP FW: Unsupported protocol version of  has been received when parsing TCBeginMessage",
                                        lua_hmap);
                                return;
                            }
                        }
                        ...
                        if (message.getCalledPartyAddress() != null
                                && message.getCalledPartyAddress().getGlobalTitle() != null
                                && oc != null && oc.getLocalOperationCode() != OC_DTLS_HANDSHAKE_CLIENT
                                && oc.getLocalOperationCode() != OC_DTLS_HANDSHAKE_SERVER) {

                            // DTLS decryption
                            if (oc.getLocalOperationCode() == OC_DTLS_DATA && dtls_engine_permanent_server.containsKey(
                                    getGTPrefix(message.getCallingPartyAddress().getGlobalTitle().getDigits()))) {

                                AbstractMap.SimpleEntry<SccpDataMessage, String> p = ss7DTLSDecrypt(message, comps,
                                        dtls_engine_permanent_server.get(getGTPrefix(
                                                message.getCallingPartyAddress().getGlobalTitle().getDigits())));
                                if (p != null) {
                                    ...
                                    if (!r.equals("")) {
                                        ...
                                        needDTLSHandshakeReason = "needDTLSHandshake indicated, because failed to decrypt Invoke message from GT: "
                                                + message.getCallingPartyAddress().getGlobalTitle().getDigits();

                                        firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message, r, lua_hmap);
                                        return;
                                    }
                                    if (!dtls_engine_expiring_server.containsKey(getGTPrefix(
                                            message.getCallingPartyAddress().getGlobalTitle().getDigits()))) {
                                        needDTLSHandshake = true;

                                        needDTLSHandshakeReason = "needDTLSHandshake indicated, because session has expired for GT: "
                                                + message.getCallingPartyAddress().getGlobalTitle().getDigits();
                                    }
                             ...
                             * needDTLSHandshakeReason =
                             * "needDTLSHandshake indicated, because no DTLS engine, but recieved Request with DTLS encrypted data from GT: "
                             * + message.getCalledPartyAddress().getGlobalTitle().getDigits();
                             * 
                             * needDTLSHandshake = true;
                             * }
                            ...
                            // Asymmetric decryption
                                ...
                                KeyPair keyPair = SS7FirewallConfig.simpleWildcardFind(
                                        SS7FirewallConfig.called_gt_decryption,
                                        message.getCalledPartyAddress().getGlobalTitle().getDigits());
                                if (keyPair != null) {
                                    AbstractMap.SimpleEntry<SccpDataMessage, String> p = crypto.tcapDecrypt(message,
                                            comps, SS7Firewall.sccpMessageFactory, keyPair);
                                    ...
                                    if (p != null) {
                                        ...
                                        String r = p.getValue();
                                        if (!r.equals("")) {
                                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message, r,
                                                    lua_hmap);
                                            return;
                                        }
                                    }
                        ...
                        if (needDTLSHandshake
                            ...
                            if (!dtls_handshake_timer
                                    .containsKey(message.getCallingPartyAddress().getGlobalTitle().getDigits())) {
                                // Only if no handshaking is ongoing
                                if (!dtls_engine_handshaking_client.containsKey(
                                        getGTPrefix(message.getCallingPartyAddress().getGlobalTitle().getDigits()))) {

                                    logger.info("Initiate DTLS handshake client side, backwards towards Calling GT: "
                                            + message.getCallingPartyAddress().getGlobalTitle().getDigits());
                                    logger.info("Initiate DTLS handshake reason: " + needDTLSHandshakeReason);

                                    final String calling_gt = String
                                            .valueOf(message.getCallingPartyAddress().getGlobalTitle().getDigits());
                                    ...
                                    Thread t = new Thread(new Runnable() {
                                        ...
                                        public void run() {
                                            try {
                                                ...
                                                // Create engine
                                                try {
                                                    dtls_engine_handshaking_client.put(getGTPrefix(calling_gt),
                                                            dtls_createSSLEngine(true));

                                                } catch (Exception ex) {
                                                    java.util.logging.Logger.getLogger(SS7Firewall.class.getName())
                                                            .log(Level.SEVERE, null, ex);
                                                }

                                                // Create socket if does not exist
                                                if (!datagramOverSS7Socket_inbound_client
                                                        .containsKey(getGTPrefix(calling_gt))) {
                                                    datagramOverSS7Socket_inbound_client.put(getGTPrefix(calling_gt),
                                                            new ConcurrentLinkedQueue<DatagramOverSS7Packet>());
                                                }

                                                dtls_handshake(
                                                        dtls_engine_handshaking_client.get(getGTPrefix(calling_gt)),
                                                        datagramOverSS7Socket_inbound_client
                                                                .get(getGTPrefix(calling_gt)),
                                                        m, opc, dpc, sls, ni, calling_gt, "client", false);
                                            } catch (Exception ex) {
                                                java.util.logging.Logger.getLogger(SS7Firewall.class.getName())
                                                        .log(Level.SEVERE, null, ex);
                                            }
                                ...
                                logger.debug("dtls_sessions_reverse.put "
                                        + message.getCallingPartyAddress().getGlobalTitle().getDigits() + " " + null);
                                dtls_handshake_timer.put(message.getCallingPartyAddress().getGlobalTitle().getDigits(),
                                        null);
                        ...
                        if (message.getCallingPartyAddress() != null
                                ...
                                && oc != null && oc.getLocalOperationCode() != OC_DTLS_HANDSHAKE_CLIENT
                                && oc.getLocalOperationCode() != OC_DTLS_HANDSHAKE_SERVER
                                && oc.getLocalOperationCode() != OC_DTLS_DATA) {
                            // --------------- TCAP verify ---------------
                            int signature_ok = -1; // no key
                            PublicKey publicKey = SS7FirewallConfig.simpleWildcardFind(
                                    SS7FirewallConfig.calling_gt_verify,
                                    message.getCallingPartyAddress().getGlobalTitle().getDigits());
                            if (publicKey != null) {
                                signature_ok = crypto.tcapVerify(message, tcb, comps, publicKey);
                                if (signature_ok == 0) {
                                    // Drop not correctly signed messages
                                    // logger.info("============ Wrong TCAP signature, message blocked. Calling GT =
                                    // " + message.getCallingPartyAddress().getGlobalTitle().getDigits() + "
                                    // ============");

                                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                            "TCAP FW: Wrong TCAP signature", lua_hmap);
                                    return;
                                }
                                // Change back to SCCP UDT from XUDT if possible
                                if (message.getData().length < 240) {
                    ...
                    case TCEndMessage._TAG:
                        try {
                            ...
                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                    "TCAP FW: ParseException when parsing TCEndMessage", lua_hmap);
                            return;
                        }
                    ...
                    case TCAbortMessage._TAG:
                        try {
                            ...
                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                    "TCAP FW: ParseException when parsing TCAbortMessage", lua_hmap);
                            return;
                        }
                    ...
                    case TCUniMessage._TAG:
                        try {
                            ...
                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                    "TCAP FW: ParseException when parsing TCUniMessage", lua_hmap);
                            return;
                        }
                        ...
                        switch (comp.getType()) {
                            case Invoke:
                                ...
                                if (oc.getLocalOperationCode() == OC_AUTO_ENCRYPTION
                                        ...
                                        && SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.hplmn_gt,
                                                message.getCalledPartyAddress().getGlobalTitle().getDigits())
                                        && !SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.hplmn_gt,
                                                message.getCallingPartyAddress().getGlobalTitle().getDigits())) {

                                    KeyPair myKeyPair = SS7FirewallConfig.simpleWildcardFind(
                                            SS7FirewallConfig.called_gt_decryption,
                                            message.getCalledPartyAddress().getGlobalTitle().getDigits());
                                    String key = SS7FirewallConfig.simpleWildcardKeyFind(
                                            SS7FirewallConfig.called_gt_decryption,
                                            message.getCalledPartyAddress().getGlobalTitle().getDigits());
                                    if (myKeyPair != null) {
                                        ...
                                        SccpDataMessage m = SS7Firewall.sccpMessageFactory.createDataMessageClass0(
                                                message.getCallingPartyAddress(), message.getCalledPartyAddress(),
                                                message.getData(), message.getOriginLocalSsn(), false, null, null);
                                        m.setData(_d);
                                ...
                                if ((oc.getLocalOperationCode() == OC_DTLS_HANDSHAKE_CLIENT
                                    ...
                                    logger.info("Received DTLS handshake message from GT: "
                                            + message.getCallingPartyAddress().getGlobalTitle().getDigits());
                                    ...
                                    if (oc.getLocalOperationCode() == OC_DTLS_HANDSHAKE_CLIENT) {
                                        ...
                                        if (!datagramOverSS7Socket_inbound_server.containsKey(getGTPrefix(
                                                message.getCallingPartyAddress().getGlobalTitle().getDigits()))) {
                                            datagramOverSS7Socket_inbound_server.put(
                                                    getGTPrefix(message.getCallingPartyAddress().getGlobalTitle()
                                                            .getDigits()),
                                                    new ConcurrentLinkedQueue<DatagramOverSS7Packet>());
                                        }
                                        ...
                                        if (params != null && params.length >= 1) {
                                            ...
                                            datagramOverSS7Socket_inbound_server
                                                    .get(getGTPrefix(message.getCallingPartyAddress().getGlobalTitle()
                                                            .getDigits()))
                                                    .add(new DatagramOverSS7Packet(
                                                            message.getCallingPartyAddress().getGlobalTitle()
                                                                    .getDigits(),
                                                            new DatagramPacket(p1.getData(), p1.getData().length)));
                                        ...
                                        try {
                                            ...
                                            if (!dtls_engine_handshaking_server.containsKey(getGTPrefix(
                                                    message.getCallingPartyAddress().getGlobalTitle().getDigits()))) {
                                                needHandshake = true;

                                                needHandshakeReason = "needDTLSHandshake indicated, because new handshaking client detected. Peer: "
                                                        + message.getCallingPartyAddress().getGlobalTitle().getDigits();
                                            }
                                            // no thread exist
                                            /*
                                             ...
                                             * 
                                             * needHandshakeReason =
                                             * "Initiate DTLS, because handshaking thread does not exist anymore. Peer: "
                                             * + orig_realm;
                                             * }
                                             */
                                            /*
                                             ...
                                             * 
                                             * needHandshakeReason =
                                             * "Initiate DTLS, because handshaking thread is not alive. Peer: " +
                                             * orig_realm;
                                             * }
                                             */
                                            /*
                                             * // NOT_HANDSHAKING status
                                             * else if (dtls_engine.get(orig_realm).getHandshakeStatus() ==
                                            ...
                                            if (needHandshake) {
                                                ...
                                                if (/*
                                                     * (!dtls_handshake_treads.containsKey(orig_realm) ||
                                                     * !dtls_handshake_treads.get(orig_realm).isAlive())
                                                     * &&
                                                     */!dtls_engine_handshaking_server.containsKey(
                                                        getGTPrefix(getGTPrefix(message.getCallingPartyAddress()
                                                                .getGlobalTitle().getDigits())))) {

                                                    logger.info("Initiate DTLS handshake server side for peer: "
                                                            + message.getCallingPartyAddress().getGlobalTitle()
                                                                    .getDigits());
                                                    logger.info(
                                                            "Initiate DTLS handshake reason: " + needHandshakeReason);

                                                    final String calling_gt = String.valueOf(message
                                                    ...
                                                    Thread t = new Thread(new Runnable() {
                                                        ...
                                                        public void run() {
                                                            try {
                                                                dtls_engine_handshaking_server.put(
                                                                        getGTPrefix(calling_gt),
                                                                        dtls_createSSLEngine(false));

                                                                dtls_handshake(
                                                                        dtls_engine_handshaking_server
                                                                                .get(getGTPrefix(calling_gt)),
                                                                        datagramOverSS7Socket_inbound_server
                                                                                .get(getGTPrefix(calling_gt)),
                                                                        /* datagramOverDiameterSocket_outbound */ m,
                                                                        opc, dpc, sls, ni, calling_gt, "server", false);
                                                            } catch (Exception ex) {
                                                                java.util.logging.Logger
                                                                        .getLogger(SS7Firewall.class.getName())
                                                                        .log(Level.SEVERE, null, ex);
                                                            }
                                                        }
                                                    });
                                    ...
                                    // Answer (server -> client)
                                        ...
                                        // Create socket if does not exists
                                        if (!datagramOverSS7Socket_inbound_client.containsKey(getGTPrefix(
                                                message.getCallingPartyAddress().getGlobalTitle().getDigits()))) {
                                            datagramOverSS7Socket_inbound_client.put(
                                                    getGTPrefix(message.getCallingPartyAddress().getGlobalTitle()
                                                            .getDigits()),
                                                    new ConcurrentLinkedQueue<DatagramOverSS7Packet>());
                                        }
                                ...
                                if (oc.getOperationType() == OperationCodeType.Global) {
                                    ...
                                    // logger.info("============ TCAP Blocked Operation Code = " +
                                    // oc.getStringValue() + " ============");
                                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                            "TCAP FW (Cat1 global opcode): OC = " + oc.getStringValue(), lua_hmap);
                                    return;
                                }
                                ...
                                if (oc != null) {
                                    ...
                                    if (SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.tcap_oc_blacklist,
                                            oc.getStringValue())) {
                                        // logger.info("============ TCAP Blocked Operation Code = " +
                                        // oc.getStringValue() + " ============");
                                        firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                                "TCAP FW (Cat1): OC = " + oc.getStringValue(), lua_hmap);
                                        return;
                                    }
                                }
                                ...
                                if (tag == TCBeginMessage._TAG && ACN == null) {
                                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                            "TCAP FW (Cat1): Missing AC", lua_hmap);
                                    return;
                                }
                                ...
                                if (oc != null
                                        && message.getCalledPartyAddress() != null
                                        && message.getCalledPartyAddress().getGlobalTitle() != null
                                        && message.getCalledPartyAddress().getGlobalTitle().getDigits() != null
                                        && message.getCallingPartyAddress() != null
                                        && message.getCallingPartyAddress().getGlobalTitle() != null
                                        && message.getCallingPartyAddress().getGlobalTitle().getDigits() != null) {
                                    // Optimization, decode MAP only if towards HPLMN and not originated from HPLMN
                                    if (SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.hplmn_gt,
                                            message.getCalledPartyAddress().getGlobalTitle().getDigits())
                                            && !SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.hplmn_gt,
                                                    message.getCallingPartyAddress().getGlobalTitle().getDigits())) {
                                        ...
                                        if (parameter != null) {
                                            ...
                                            if (oc.getStringValue().equals("3")) {
                                                CancelLocationRequestImpl ind = new CancelLocationRequestImpl(
                                                        MAPApplicationContext.getProtocolVersion(ACN.getOid()));
                                                try {
                                                    ind.decodeData(map_ais, buf.length);
                                                } catch (MAPParsingComponentException ex) {
                                                    // java.util.logging.Logger.getLogger(SS7Firewall.class.getName()).log(Level.SEVERE,
                                                    // null, ex);
                                                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                                                            "MAP FW: Failed decoding, OC = " + oc.getStringValue(),
                                                            lua_hmap);
                                                    return;
                                                }
                                            ...
                                            if (oc.getStringValue().equals("4")) {
                                                ProvideRoamingNumberRequestImpl ind = new ProvideRoamingNumberRequestImpl(
                                                        MAPApplicationContext.getProtocolVersion(ACN.getOid()));
                                                try {
                                                    ind.decodeData(map_ais, buf.length);
                                                } catch (MAPParsingComponentException ex) {
                                            ...
                                            if (oc.getStringValue().equals("7")) {
                                                InsertSubscriberDataRequestImpl ind = new InsertSubscriberDataRequestImpl(
                                                        MAPApplicationContext.getProtocolVersion(ACN.getOid()));
                                                try {
                                                    ind.decodeData(map_ais, buf.length);
                                                } catch (MAPParsingComponentException ex) {
                                            ...
                                            if (imsi != null) {
                                                ...
                                                for (String imsi_prefix : SS7FirewallConfig.hplmn_imsi.keySet()) {
                                                    if (imsi.startsWith(imsi_prefix)) {
                                                        // logger.info("============ MAP Cat2 Blocked Operation Code = "
                                                        // + oc.getStringValue() + " ============");
                                                        firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt,
                                                                message, "MAP FW (Cat2): OC = " + oc.getStringValue(),
                                                                lua_hmap);
                                                        return;
                                                    }
                                                }
                            ...
                            case ReturnResultLast:
                                ...
                                if (oc.getLocalOperationCode() == OC_AUTO_ENCRYPTION
                                    ...
                                    if (encryption_autodiscovery_sessions.containsKey(
                                            message.getCallingPartyAddress().getGlobalTitle().getDigits().substring(0,
                                                    Math.min(encryption_autodiscovery_digits,
                                                            message.getCallingPartyAddress().getGlobalTitle()
                                                                    .getDigits().length())))
                                            && encryption_autodiscovery_sessions.get(message.getCallingPartyAddress()
                                                    .getGlobalTitle().getDigits().substring(0,
                                                            Math.min(encryption_autodiscovery_digits,
                                                                    message.getCallingPartyAddress().getGlobalTitle()
                                                                            .getDigits().length()))) == dialogId) {

                                        // do not remove the key and wait for expiration to not send too many
                                        // autodiscovery request messages
                                        // encryption_autodiscovery_sessions.remove(message.getCallingPartyAddress().getGlobalTitle().getDigits().substring(0,
                                        // Math.min(encryption_autodiscovery_digits,
                                        // message.getCallingPartyAddress().getGlobalTitle().getDigits().length())));
                ...
                if (externalFirewallRules.ss7FirewallRules(message) == false) {
                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                            "SS7 FW: Match with Externel Firewall rules", lua_hmap);
                    return;
                }
                ...
                for (i = 0; i < SS7FirewallConfig.lua_blacklist_rules.size(); i++) {
                    try {
                        ...
                        lua_match |= r;
                        if (r) {
                            // logger.debug("============ LUA rules blacklist: " +
                            // SS7FirewallConfig.lua_blacklist_rules.get(i) + " ============");
                            // logger.debug("============ LUA variables ============");
                            // for (String key : lua_hmap.keySet()) {
                ...
                if (lua_match) {
                    firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message,
                            "MAP FW:  Match with Lua rule " + i, lua_hmap);
                    return;
                }
                // ------------------------------------------
                ...
                if (connectorIDS != null) {
                    EncodingResultData erd;
                    try {
                        erd = ((SccpMessageImpl) message).encode(SS7Firewall.sccpStack, lmrt,
                                mup.getMaxUserDataLength(dpc), logger, SS7Firewall.sccpStack.isRemoveSpc(),
                                SS7Firewall.sccpStack.getSccpProtocolVersion());
                        if (connectorIDS
                                .evalSCCPMessage(DatatypeConverter.printHexBinary(erd.getSolidData())) == false) {
                            firewallMessage(mup, mupReturn, opc, dpc, sls, ni, lmrt, message, "MAP FW:  Blocked by IDS",
                                    lua_hmap);
                            return;
                        }
                    } catch (ParseException ex) {
                ...
                // ------------ TCAP encryption -------------
                boolean signing_required = false;
                if (message.getCalledPartyAddress() != null
                        && message.getCalledPartyAddress().getGlobalTitle() != null) {
                    // DTLS encryption
                    if (dtls_engine_permanent_client
                            .containsKey(getGTPrefix(message.getCalledPartyAddress().getGlobalTitle().getDigits()))) {

                        AbstractMap.SimpleEntry<SccpDataMessage, LongMessageRuleType> p = ss7DTLSEncrypt(message,
                                dtls_engine_permanent_client
                                        .get(getGTPrefix(message.getCalledPartyAddress().getGlobalTitle().getDigits())),
                                lmrt);
                        ...
                        if (m == null) {
                            ...
                            dtls_engine_expiring_client
                                    .remove(getGTPrefix(message.getCalledPartyAddress().getGlobalTitle().getDigits()));
                        }

                        message = m;
                    }
                    // Asymmetric encryption
                    else if (SS7FirewallConfig.simpleWildcardCheck(SS7FirewallConfig.called_gt_encryption,
                            message.getCalledPartyAddress().getGlobalTitle().getDigits())) {
                        PublicKey publicKey = SS7FirewallConfig.simpleWildcardFind(
                                SS7FirewallConfig.called_gt_encryption,
                                message.getCalledPartyAddress().getGlobalTitle().getDigits());
                        if (publicKey != null) {
                            AbstractMap.SimpleEntry<SccpDataMessage, LongMessageRuleType> p = crypto
                                    .tcapEncrypt(message, sccpMessageFactory, publicKey, lmrt);
                            message = p.getKey();
                            lmrt = p.getValue();
                        }
                    }
                ...
                if (signing_required && tag == TCBeginMessage._TAG) {
                    KeyPair keyPair = SS7FirewallConfig.simpleWildcardFind(SS7FirewallConfig.calling_gt_signing,
                            message.getCallingPartyAddress().getGlobalTitle().getDigits());
                    if (keyPair != null) {
                ...
                if (message.getCalledPartyAddress() != null
                    ...
                    // If not encrypted Requests towards non HPLMN
                        ...
                        if (!dtls_handshake_timer.containsKey(_called_gt)) {
                            ...
                            if (!dtls_engine_expiring_client.containsKey(getGTPrefix(_called_gt))
                                    && !dtls_engine_handshaking_client.containsKey(getGTPrefix(_called_gt))) {
                                    ...
                                    Thread t = new Thread(new Runnable() {
                                        ...
                                        public void run() {
                                            try {
                                                ...
                                                dtls_engine_handshaking_client.put(getGTPrefix(called_gt),
                                                        dtls_createSSLEngine(true));

                                                // Create socket if does not exists
                                                if (!datagramOverSS7Socket_inbound_client
                                                        .containsKey(getGTPrefix(called_gt))) {
                                                    datagramOverSS7Socket_inbound_client.put(getGTPrefix(called_gt),
                                                            new ConcurrentLinkedQueue<DatagramOverSS7Packet>());
                                                }

                                                dtls_handshake(
                                                        dtls_engine_handshaking_client.get(getGTPrefix(called_gt)),
                                                        datagramOverSS7Socket_inbound_client
                                                                .get(getGTPrefix(called_gt)),
                                                        m, dpc, opc, sls, ni, called_gt, "client", true);
                                            } catch (Exception ex) {
                                                java.util.logging.Logger.getLogger(SS7Firewall.class.getName())
                                                        .log(Level.SEVERE, null, ex);
                                            }
                    ...
                    // only if not towards HPLMN
                        ...
                        if (!encryption_autodiscovery_sessions.containsKey(message.getCalledPartyAddress()
                                .getGlobalTitle().getDigits().substring(0, Math.min(encryption_autodiscovery_digits,
                                        message.getCallingPartyAddress().getGlobalTitle().getDigits().length())))) {
                            logger.debug("============ Preparing Autodiscovery Invoke ============ ");

                            TCBeginMessage t = TcapFactory.createTCBeginMessage();

                            byte[] otid = { (byte) randomGenerator.nextInt(256), (byte) randomGenerator.nextInt(256),
                                    (byte) randomGenerator.nextInt(256), (byte) randomGenerator.nextInt(256) };

                            // to this as soon as possible to prevent concurrent threads to duplicate the
                            // autodiscovery
                            encryption_autodiscovery_sessions.put(
                                    message.getCalledPartyAddress().getGlobalTitle().getDigits().substring(0,
                                            Math.min(encryption_autodiscovery_digits, message.getCallingPartyAddress()
                                                    .getGlobalTitle().getDigits().length())),
                                    Utils.decodeTransactionId(otid));
                            ...
                            try {
                                ...
                                java.util.logging.Logger.getLogger(SS7Firewall.class.getName()).log(Level.SEVERE, null,
                                        ex);
                            }
                            ...
                            LongMessageRuleType l = lmrt;
                            SccpDataMessage m = SS7Firewall.sccpMessageFactory.createDataMessageClass0(
                                    message.getCalledPartyAddress(), message.getCallingPartyAddress(),
                                    message.getData(), message.getOriginLocalSsn(), false, null, null);
                            m.setData(_d);
                ...
                if (message.getData().length >= 240) {
                    lmrt = LongMessageRuleType.XUDT_ENABLED;
                    SccpDataMessage m = SS7Firewall.sccpMessageFactory.createDataMessageClass0(
                            message.getCalledPartyAddress(), message.getCallingPartyAddress(), message.getData(),
                            message.getOriginLocalSsn(), false, null, null);
                    message = m;
                }
                sendSccpMessage(mup, opc, dpc, sls, ni, lmrt, message);
            }
    ...
    public static void main(String[] args) {
         ...
         * } catch (ScriptException ex) {
         * java.util.logging.Logger.getLogger(SS7Firewall.class.getName()).log(Level.
         * SEVERE, null, ex);
         * }
         */
        ...
        // ---- IDS API -----
        try {
            String ids_api_type = (String) SS7FirewallConfig
                    .get("$.sigfw_configuration.firewall_rules.ids.ids_api_type");
            if (ids_api_type != null && ids_api_type.equals("REST")) {
                ...
                List<Map<String, Object>> ids_servers = SS7FirewallConfig
                        .get("$.sigfw_configuration.firewall_rules.ids.ids_servers");
        ...
        // ---- mThreat API -----
        try {
            String mthreat_api_type = (String) SS7FirewallConfig
                    .get("$.sigfw_configuration.firewall_rules.mthreat.mthreat_api_type");
            if (mthreat_api_type != null && mthreat_api_type.equals("REST")) {
                ...
                List<Map<String, Object>> ids_servers = SS7FirewallConfig
                        .get("$.sigfw_configuration.firewall_rules.mthreat.mthreat_servers");
                for (int i = 0; i < ids_servers.size(); i++) {
                    // connectorIDS.addServer("https://localhost:8443", "user", "password");
    ...
    public void onProvideSubscriberLocationRequest(
            ProvideSubscriberLocationRequest provideSubscriberLocationRequestIndication) {
        // throw new UnsupportedOperationException("Not supported yet."); //To change
        // body of generated methods, choose Tools | Templates.
    ...
    public void onSubscriberLocationReportRequest(
            SubscriberLocationReportRequest subscriberLocationReportRequestIndication) {
        // throw new UnsupportedOperationException("Not supported yet."); //To change
        // body of generated methods, choose Tools | Templates.
    ...
    public void onSubscriberLocationReportResponse(
            SubscriberLocationReportResponse subscriberLocationReportResponseIndication) {
        // throw new UnsupportedOperationException("Not supported yet."); //To change
        // body of generated methods, choose Tools | Templates.
    ...
    public void onSendRoutingInfoForLCSResponse(
            SendRoutingInfoForLCSResponse sendRoutingInforForLCSResponseIndication) {
        // throw new UnsupportedOperationException("Not supported yet."); //To change
        // body of generated methods, choose Tools | Templates.
    ...
    SSLContext dtls_getDTLSContext() throws Exception {
        ...
        // TrustManager[] { tm }, /*null*/new java.security.SecureRandom());
    ...
    void dtls_handshake(SSLEngine engine,
        ...
        while (!endLoops && System.currentTimeMillis() < _end/*
                                                              * &&
                                                              * (dtls_serverException == null) && (dtls_clientException
                                                              * == null)
                                                              */) {
            ...
            if (hs == SSLEngineResult.HandshakeStatus.NEED_UNWRAP ||
                ...
                if (hs == SSLEngineResult.HandshakeStatus.NEED_UNWRAP) {
                    ...
                    DatagramOverSS7Packet packet;// = new DatagramOverSS7Packet( peer_realm, new DatagramPacket(buf,
                                                 // buf.length));
                    ...
                    if (packet == null) {
                        ...
                        for (DatagramOverSS7Packet p : packets) {
                            // printHex("Reproduced packet", p.getP().getData(), p.getP().getOffset(),
                            // p.getP().getLength());
                    ...
                    // printHex("Poll packet", packet.getP().getData(), packet.getP().getOffset(),
                    // packet.getP().getLength());
    ...
    void dtls_sendDatagramOverSS7(Mtp3UserPart mup, int dpc, int opc, int sls, int ni, String _peer_gt,
            DatagramOverSS7Packet p, String side, boolean forwardIndicator) {

        LongMessageRuleType lmrt = LongMessageRuleType.XUDT_ENABLED;
        GlobalTitle callingGT = SS7Firewall.sccpProvider.getParameterFactory().createGlobalTitle(
                SS7FirewallConfig.fw_gt.firstKey(), 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_MOBILE,
                null, NatureOfAddress.INTERNATIONAL);
        GlobalTitle calledGT = SS7Firewall.sccpProvider.getParameterFactory().createGlobalTitle(_peer_gt, 0,
                org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_MOBILE, null, NatureOfAddress.INTERNATIONAL);
        SccpAddress callingParty = SS7Firewall.sccpProvider.getParameterFactory()
                .createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, callingGT, 1, 8);
        SccpAddress calledParty = SS7Firewall.sccpProvider.getParameterFactory()
                .createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, calledGT, 2, 8);

        byte[] data = { 0 };
        SccpDataMessage message = SS7Firewall.sccpMessageFactory.createDataMessageClass0(calledParty, callingParty,
                data, 8, false, null, null);
        ...
        LongMessageRuleType l = lmrt;
        SccpDataMessage m = SS7Firewall.sccpMessageFactory.createDataMessageClass0(message.getCalledPartyAddress(),
                message.getCallingPartyAddress(), message.getData(), message.getOriginLocalSsn(), false, null, null);
        m.setData(_d);
     ...
     * @return AbstractMap.SimpleEntry<message, lmrt> - message and indicator if UDT
     *         or XUDT should be send
     */
    public AbstractMap.SimpleEntry<SccpDataMessage, LongMessageRuleType> ss7DTLSEncrypt(SccpDataMessage message,
            SSLEngine engine, LongMessageRuleType lmrt) {
        ...
        try {
            ...
            SccpDataMessage m = sccpMessageFactory.createDataMessageClass0(message.getCalledPartyAddress(),
                    message.getCallingPartyAddress(), _d, message.getOriginLocalSsn(), false, null, null);
            l = LongMessageRuleType.XUDT_ENABLED;
     ...
     * @param engine
     * @return AbstractMap.SimpleEntry<message, result> - message and result
     *         indicator
     */
    public AbstractMap.SimpleEntry<SccpDataMessage, String> ss7DTLSDecrypt(SccpDataMessage message, Component[] comps,
            SSLEngine engine) {
        ...
        try {
            ...
            SccpDataMessage m = sccpMessageFactory.createDataMessageClass0(message.getCalledPartyAddress(),
                    message.getCallingPartyAddress(), decryptedText, message.getOriginLocalSsn(), false, null, null);
